...
if (!$superview) {
	...
	if (!isset($_POST["mode"])) {
		...
		if (!$query) $msg .= "Database Error : ".$q;
			...
			$i = 0;
			while ($row = mysql_fetch_row($query)) {
				preg_match("/([0-9]{4})-([0-9]{2})-([0-9]{2}) ([0-9]{2}):([0-9]{2}):[0-9]{2}/i",$row[1],$start);
				...
				if ($row[2]) {
					preg_match("/([0-9]{4})-([0-9]{2})-([0-9]{2}) ([0-9]{2}):([0-9]{2}):[0-9]{2}/i",$row[2],$end);
					if ($end[4] == "00") {
						$end[4] = 12;
						$end_ap = "am";
					} elseif ($end[4] >= 12) {
		...
		if (!$query) $msg .= "Database Error : ".$q;
			...
			while ($_POST["date_".$j]) {
				if (!$_POST["delete_".$j]) {
					...
					if (!preg_match ("/([0-9]{1,2})[\/-]+([0-9]{1,2})[\/-]+([0-9]{4})/",$_POST["date_".$j],$date)) {
			...
			if ($_POST["date_add"]) {
				...
				if (!preg_match ("/([0-9]{1,2})[\/-]+([0-9]{1,2})[\/-]+([0-9]{4})/",$_POST["date_add"],$date)) {
...
if ($_POST["date_add"]) {
	...
	//loop through recurring events
	preg_match ("/([0-9]{1,2})[\/-]+([0-9]{1,2})[\/-]+([0-9]{4})/",$_POST["date_add"],$date);
	$juno = 0;
	$r = $_POST["recurring"];
	if (!$r) $r = 0;
	$int = $_POST["interval"];
